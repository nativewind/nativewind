...
// Update styles if prop changes
cssInterop(component, { "new-prop": "existing-prop" }, ['prop']);
```
...
Here is an example of forcing a re-render with style update based on prop change using `<Pressable />`. This is useful for the `disabled:` pseudo selector
