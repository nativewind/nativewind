...
When compiling for native platforms, NativeWind uses the Tailwind PostCSS plugin to process your styles to CSS and then applies the NativeWind plugin to compile the CSS to the parameters for `NativeWindStyleSheet.create`. The CSS generated by Tailwind is slightly modifed by Tailwind plugins for easier processing.
